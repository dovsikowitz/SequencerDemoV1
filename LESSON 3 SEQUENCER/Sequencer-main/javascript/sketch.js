...
let rows = 10;         // Number of rows (notes)
let cols = 32;        // Number of columns (steps)
let grid = [];
let currentStep = 0;
let synths = [];
let isPlaying = false;
...
function setupSynths() {
  ...
  chorus = new Tone.Chorus({frequency: 5, delayTime: 2.5, depth: 0.8, spread: 200}).start();
  ...
  reverb = new Tone.Reverb({
    ...
    for(let i = 0; i < rows; i++) {
      let synth = new Tone.Synth().connect(chorus).connect(reverb);
      synths.push(synth);
    }
  }
...
function setup() {
  ...
  reverbMixSlider = createSlider(0,1,0.5,0.01);
  reverbMixSlider.position(sequencerX + sequencerWidth / 2- 100, sequencerY + sequencerHeight + 25);
  reverbMixSlider.style('width', '200px');
  reverbMixSlider.addClass("slider");

  chorusFeedbackSlider = createSlider(0,1, 0.5, 0.01);
  chorusFeedbackSlider.position(sequencerX + sequencerWidth / 2- 100, sequencerY + sequencerHeight + 80);
  chorusFeedbackSlider.style('width', '200px');
  chorusFeedbackSlider.addClass("slider");
  ...
  }
{
  ...
  let chorusFeedback = chorusFeedbackSlider.value();
  chorus.depth = chorusFeedback;
}{

  let reverbMix = reverbMixSlider.value();
